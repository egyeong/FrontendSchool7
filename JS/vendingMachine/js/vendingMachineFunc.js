class VendingMachineFunc {
  ...
  // 장바구니에 선택한 콜라의 목록을 생성하는 함수
  stagedItemGenerator(target) {
    const stagedItem = document.createElement("li");
    stagedItem.dataset.item = target.dataset.item;
    stagedItem.dataset.img = target.dataset.img;
    stagedItem.dataset.price = target.dataset.price;
    stagedItem.innerHTML = `<img src="./img/${target.dataset.img}" alt="">
    ${target.dataset.item}
    <strong>
      1
  ...
  bindEvents() {
    ...
    this.btnPut.addEventListener("click", (event) => {
      ...
      if (inputCost) {
        ...
        if (inputCost <= myMoneyVal && inputCost > 0) {
          // 현재 소지금의 값, Intl 객체를 통해서 사용자 국가에서 사용하는 포맷으로 숫자를 변경합니다(단위).
     ...
     * 3. 자판기 메뉴 기능
     * 콜라를 누르면, 잔액 === 잔액 - 콜라 가격
     * 콜라 가격보다 잔액이 적다면, "잔액이 부족합니다." 라는 경고창 출력
