...
function test() {
  ...
  console.log("hello");
  return;
  // 아래처럼 에러가 나는 코드가 있어도, js엔진이 보지 않는 공간에서는 에러를 출력하지 않습니다.
  console.loga("hello");
  console.logb("hello");
  console.logc("hello");
}
...
let value = true;
let result4 = true || errorCode(); // true, errorCode()를 실행시키지 않았다.

///////////////

// return : 아래 코드는 반환보다는 종료의 느낌으로 쓰는 코드입니다.
function text() {
  if (true) {
    if (true) {
      return;
...
// 2. 함수를 리턴값으로 전달(스코프와 클로저를 배우고나서 다시 다룰 예정입니다.)
function one(x) {
  function two(y) {
    return x + y;
  }
